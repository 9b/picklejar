...
class brine(object):
	def __init__(self, scope=[], jar=PickleJar()):
		self.__locals = {}
		self.__scope = scope
		self.__jar = jar
	...
	def __handle(self, context):
		if '__all__' in self.__scope:
			for vname, value in context.__locals.iteritems():
				try: context.__jar.put(vname, value)
				except: pass
			return True
		
		for item in self.__scope:
			if context.__locals.has_key(item):
				try: context.__jar.put(item, context.__locals[item])
				except: pass
			return True
